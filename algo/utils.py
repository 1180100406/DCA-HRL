...
def get_mbrl_fetch_reward_function(env, env_name, binary_reward, absolute_goal):
    ...
    if env_name in ["FetchPickAndPlace-v1", "FetchPush-v1", 'HandReach-v0', 'FetchSlide-v1']:
        action_penalty_coeff = 0.1
        distance_threshold = 0.05
    if env_name in ["Reacher3D-v0", "Pusher-v0", "FetchPickAndPlace-v1", "FetchPush-v1", 'HandReach-v0', 'FetchReach-v1', 'FetchSlide-v1']:
...
class PriorityQueue:
    ...
    def add_list(self, state_list, achieved_goal_list, score_list, a_net=None):
        ...
        def _score_to_float(score):
            # scores can arrive as GPU tensors; normalize to a Python float for set/hash ops
            if torch.is_tensor(score):
                return float(score.detach().cpu().item())
            return float(score)
        new_elems = [StorageElement(state=state, achieved_goal=achieved_goal, score=_score_to_float(score))
