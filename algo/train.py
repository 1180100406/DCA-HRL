...
def run_higl(args):
    ...
    if args.env_name in ["Reacher3D-v0"]:
        ...
        high = np.array((10., 10.))
        low = - high
    elif args.env_name in ["FetchReach-v1"]:
        high = np.array([0.8, 0.8, 0.8])
        low = - high
    elif args.env_name == "antmaze-medium-navigate-v0":
        high = np.array([20., 20.])
        low = - high
    elif args.env_name == "pointmaze-large-navigate-oraclerep-v0" or args.env_name == 'pointmaze-teleport-navigate-oraclerep-v0':
        high = np.array([30., 20.])
        low = - high
    else:
    ...
    if "Ant" in args.env_name or 'Fetch' in args.env_name or 'Pusher' in args.env_name or 'Reacher' in args.env_name:
        ...
        action_dim = env.action_space.shape[0]
        goal, next_goal = info['goal'], info['goal']
        goal_dim = goal.shape[0]
        state = obs
        achieved_goal = state[:goal_dim]
    ...
    if args.env_name in ["Reacher3D-v0", "Pusher-v0", "FetchPickAndPlace-v1", "FetchPush-v1", 'HandReach-v0', "FetchReach-v1", 'FetchSlide-v1']:
    ...
    while total_timesteps < args.max_timesteps:
        ...
        if "Ant" in args.env_name or 'Fetch' in args.env_name  or 'Pusher' in args.env_name or 'Reacher' in args.env_name:
            ...
            next_state = obs
            next_goal = goal
            next_achieved_goal = obs[:goal_dim]
